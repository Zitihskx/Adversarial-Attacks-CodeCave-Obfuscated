...
#malconv = torch.load('checkpoint/example_sd_123.model', map_location=torch.device('cuda'))
malconv = torch.load('/home/user/Desktop/Malconv2/MalConv_channels_128_filterSize_512_stride_512_embdSize_8/epoch_9.checkpoint', map_location=torch.device('cuda'))
...
for _, val_batch_data in enumerate(validloader):
    ...
    data = exe_input[0].to('cuda')  # Use 'cuda:0' or other GPU device identifier if necessary
...
# Convert the GPU tensor to a NumPy array
    ...
    perturbation_length = int(0.15*int(length))
    data = np.concatenate([data, np.random.randint(0, 256, perturbation_length)])
    ...
    # with open("Performance_original_data.csv", 'a') as file:
    #     writer = csv.writer(file)
    #     data = [val_batch_data, length, prob ]
    ...
    with open("data_15_cave_append.csv", 'a') as file:
        writer = csv.writer(file)
